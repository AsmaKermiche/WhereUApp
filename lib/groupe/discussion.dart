...
class _ChatState extends State<chat> {
      ...
      child: Row(
        children: <Widget>[
          IconButton(
              ...
              onPressed: () async {
                try {
                  File file = await _storageService.pickPhoto() ;
                  _group.sendPhoto(file, _utilisateur.sharableUserInfo.id, _utilisateur.sharableUserInfo.displayName);
                }
                catch (e){
                }
              }),
        ...
        appBar: AppBar(
          ...
          actions: <Widget>[
            IconButton(
                ...
                color: Color(0xffF2E9DB),
                onPressed: () {
                  Navigator.of(context).push(new MaterialPageRoute(builder: (BuildContext context) =>deleteConfirmation()));
                }
                )
          ],
        ),
                  ...
                  child: StreamBuilder<List<Message>>(
                    ...
                    stream: _messegesStream.map((querySnapshot) {
                      ...
                      querySnapshot.documents.forEach((document){
                        ...
                        if(!message.seenBy.contains(_utilisateur.sharableUserInfo.id)){
                          _group.setMessageSeen(_utilisateur.sharableUserInfo.id, message);
                        }
      ...
      //TODO Marwa hadi ta3 alignment mahabatch torj
        ...
        child: Column(
          children: <Widget>[
            fromMe? Row(
              children: <Widget>[
                (message.type==TypeMessage.Photo) ? photo(message.text): Text(message.text , style: TextStyle( color : Color(0xffF2E9DB)),),
                IconButton(
                  color : Color(0xffF2E9DB),
                  icon: Icon(
  ...
  AlertDialog deleteConfirmation (){
    return AlertDialog(
      ...
      content: Text(
          "Cette conversation sera entiérement supprimé, tous les messages seront perdus"),
      actions: <Widget>[
        FlatButton(
            child: Text('Annuler',
                style: TextStyle(
        ...
        FlatButton(
            ...
            onPressed: () => _group.deleteConversationFor(_utilisateur.sharableUserInfo.id) ,
        ),
      ],
    );
  }
